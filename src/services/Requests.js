...
class RequestService {
  async getRequests(filters = {}, lastRefKey = 0, limit = 10) {
    try {
      const response = await post(`/requests/getAll?lastRef=${lastRefKey}&limit=${safePaginationLimit(limit)}`, { filters })
  ...
  async editRequest(id, updateData) {
    try {
      const response = await put(`/requests/${id}`, updateData)
      if (!response || !response.data) throw new Error("An error occured. Please try again")
      return response.data
    } catch (error) {
      handleError(error)
  ...
  async createRequest(data) {
    try {
      const response = await post(`/requests`, data)
      if (!response) throw new Error("An error occured. Please try again")
      return response.data
    } catch (error) {
      handleError(error)
  ...
  async approveRequest(id) {
    try {
      const response = await get(`/requests/approve/${id}`)
      if (!response) throw new Error("An error occured. Please try again")
      return response.data
    } catch (error) {
      handleError(error)
